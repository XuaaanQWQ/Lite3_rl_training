...
class PolicyExporter(torch.nn.Module):
    ...
    def forward(self, observations_total):
        obs = observations_total[:,:137]
        obs_history = observations_total[:,137:]
...
if __name__ == '__main__':
    path = '/home/ysc/myx/Lite3_rl_training/legged_gym/logs/M20/Jun18_19-32-11_/'
    model_name = 'model_20000.pt'
    loaded_dict = torch.load(path + model_name, map_location='cpu')
    actor_critic = ActorCritic(137, # env.num_obs, 
                               66,  # env.num_previleged_obs, [54, 241]
                               40*137,  # env.num_obs_history, 
                               16,  
